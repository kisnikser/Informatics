...
//Функция делит число на 2;
LongN * div2LN(const LongN *a)
{
    int i, p = 0;
    LongN* tmp = 0;
    tmp = calloc(1, (sizeof (LongN) + (a->n - 1) * sizeof(char)));
...
// Функция перемножает два длинных числа и получаем новое число
LongN * MultLN( const LongN * a, const LongN * b)
{
    int i, j, p = 0, flag = 0;
    LongN* tmp = 0;
    LongN* piece[a->n];
    tmp = calloc(1, (sizeof (LongN) + (a->n - 1) * (b->n - 1) * sizeof(char)));
    piece = calloc(1, a->n * (sizeof (LongN) + (b->n + 1) * sizeof(char)));
    ...
    for (i = 0; i < a->n; i++)
    {
        piece[i]->sign = tmp->sign;
        piece[i]->n = b->n + 2;
        for(j = 0; j < b->n; j++)
        {
            piece[i]->dig[j] += a->dig[i] * b->dig[j] + p;
            p = piece[i]->dig[j] / 100;
            piece[i]->dig[j] %= 100;
        }
        piece[i]->dig[b->n] += p;
